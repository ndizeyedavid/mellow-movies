...
// Desktop: hosted backend is the default. VITE_API_BASE can override locally.
const API_BASE =
  import.meta.env.VITE_API_BASE ?? "https://mellow-movies.fastapicloud.dev";
...
export function proxifyMediaUrl(url: string): string {
  ...
  // Desktop Attempt A: renderer gets direct CDN via Electron main with correct Referer.
  // If this URL is already proxied, keep it (fallback B). Otherwise return direct
  // so main's webRequest / mellow://fetch can add Referer from residential IP.
  // Hosted web app would proxify here; desktop explicitly does NOT.
  const isDesktop = typeof window !== "undefined" && !!(window as unknown as { electronAPI?: unknown }).electronAPI;
  if (isDesktop) {
