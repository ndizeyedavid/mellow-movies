...
self.addEventListener("fetch", (event) => {
  ...
  if (isStreamHost) {
    // Only intercept <video>/<audio> element requests (destination video/audio).
    // JS fetch() from the blob fallback (destination "") bypasses the SW and
    // goes direct to the CDN with its own referrer option — otherwise we'd
    // double-fetch (page fetch -> SW fetch -> CDN) and retry-storm the CDN
    // into a 429 (SW retry + blob 3x retry + video parallel Ranges = 6+ reqs).
    if (request.destination !== "video" && request.destination !== "audio") {
      return; // let page JS fetch handle it directly (bypasses SW)
    }
    // Single attempt, no retry storm. Don't forward the original
    // Referer/Origin — let the `referrer` option set moviebox.ph.
    // Forward only Range/Accept. Use no-cors so a 429 (which omits CORS
    // headers) returns an opaque response instead of a console CORS error.
